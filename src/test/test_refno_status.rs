...
async fn test_get_refno_status() -> anyhow::Result<()> {
    ...
    println!("参考号 {} 在会话 {} 中的偏移是 {:#4X} 页号 {:#4X}", refno1, sesno, offset, offset/0x800);
    ...
    if let Some((current_sesno, current_offset)) = current {
        println!("参考号 {} 在当前会话 {} 中的偏移是 {:#4X}  页号 {:#4X}", refno1, current_sesno, current_offset, current_offset/0x800);
        if let Some((prev_sesno, prev_offset)) = prev {
            println!("参考号 {} 在前一个会话 {} 中的偏移是 {:#4X}  页号 {:#4X}", refno1, prev_sesno, prev_offset, prev_offset/0x800);
        }
    }
...
fn test_get_refno_operation_status() -> anyhow::Result<()> {
    ...
    for (sesno, offset) in &history {
        println!("  会话 {}: 偏移 {:#4X} 页号 {}", sesno, offset, offset / 0x800);
    }
    ...
    if history.len() > 1 {
        ...
        match status1_map.get(&refno1) {
            Some(EleOperationDetail::Add(_)) => {
                println!("参考号 {} 在最早会话 {} 中的状态为: 新增", refno1, earliest_sesno);
                // 检查通过
            },
            status => {
                println!("参考号 {} 在最早会话 {} 中的状态为: {:?}", refno1, earliest_sesno, status);
                panic!("最早会话的状态应该是Add");
            }
        }
