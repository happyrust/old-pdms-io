...
use aios_core::{RefU64, NamedAttrValue};
...
impl ElementSearchClient {
    ...
    pub async fn index_elements(&self, elements: &[EleOperationData]) -> Result<()> {
        ...
        if !documents.is_empty() {
            ...
            println!("已添加 {} 个元素到搜索索引，任务 ID: {}", documents.len(), task.task_uid);
        }
    ...
    pub async fn search_by_name(&self, name_query: &str, limit: usize) -> Result<Vec<ElementDocument>> {
        let index = self.client.index(&self.index_name);
        
        let search_result: SearchResults<ElementDocument> = index
            .search()
            .with_query(name_query)
            .with_attributes_to_highlight(meilisearch_sdk::search::Selectors::Some(&["name", "attributes_text"]))
            .with_limit(limit)
            .execute()
            .await?;
    ...
    /// # 返回值
    /// * `Result<Vec<ElementDocument>>` - 搜索结果
    pub async fn search_by_type(&self, type_query: &str, limit: usize) -> Result<Vec<ElementDocument>> {
        let index = self.client.index(&self.index_name);
    ...
    pub async fn fuzzy_search(
        ...
        // 预先构建过滤字符串以延长生命周期
        let filter_string = element_type_filter.map(|type_filter| {
            format!("element_type = {}", type_filter)
        });
        ...
        let mut search_request = search_builder
            .with_query(query)
            .with_attributes_to_highlight(meilisearch_sdk::search::Selectors::Some(&["name", "element_type", "attributes_text"]))
            .with_limit(limit);
    ...
    pub async fn search_with_stats(&self, query: &str, limit: usize) -> Result<SearchResultWrapper> {
        let index = self.client.index(&self.index_name);
        ...
        Ok(SearchResultWrapper {
            results: search_result.hits.into_iter().map(|hit| hit.result).collect(),
    ...
    pub async fn add_documents(&self, documents: &[ElementDocument]) -> Result<()> {
        let index = self.client.index(&self.index_name);
        let task = index.add_documents(documents, Some("id")).await?;
        println!("✅ 添加了 {} 个文档，任务 ID: {}", documents.len(), task.task_uid);
        Ok(())
    }
}
